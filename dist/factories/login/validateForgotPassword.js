"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.default = validateForgotPassword;
const validateFields_1 = __importDefault(require("../../utils/validateFields"));
const index_1 = require("../../services/mail/index");
const index_2 = __importDefault(require("../../services/prisma/index"));
const utils_1 = require("../../utils/utils");
function validateForgotPassword(request, response) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            const dataValidateForgotPassword = request.body;
            (0, validateFields_1.default)(['email'], dataValidateForgotPassword);
            const codEmailValidate = (0, utils_1.generateNumberRandom)();
            const storeInfo = yield index_2.default.user.findUnique({
                where: { email: dataValidateForgotPassword.email }
            });
            if (!storeInfo) {
                throw new Error('Não encontrado usuário com o e-mail informado.');
            }
            (0, index_1.sendEmailChangePass)(storeInfo.email, codEmailValidate, storeInfo.name);
            yield index_2.default.user.update({
                where: { id: storeInfo.id },
                data: { codEmailPass: codEmailValidate }
            });
            return response.json({ Success: true });
        }
        catch (error) {
            return response.status(400).json({ Success: false, erro: error.message });
        }
    });
}
