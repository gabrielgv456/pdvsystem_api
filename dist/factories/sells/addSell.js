    ...
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            yield index_1.default.$transaction((prismaTx) => __awaiter(this, void 0, void 0, function* () {
                ...
                const createSellonDB = yield prismaTx.sells.create({
                    data: {
                        storeId: sell.UserId,
                        ...
                        codRef: ((_a = nextCodRefSell === null || nextCodRefSell === void 0 ? void 0 : nextCodRefSell.codRef) !== null && _a !== void 0 ? _a : 1000) + 1
                    }
                });
                ...
                for (const product of sell.Products) {
                    ...
                    if (!searchProduct) {
                        throw new Error(`Não foi encontrado produto com o id ${product.id}`);
                    }
                    if (product.quantity > searchProduct.quantity) {
                        throw new Error('Quantidade maior do que o saldo!');
                    }
                    else {
                        yield prismaTx.products.update({
                            where: {
                                id: product.id
                            },
                            data: {
                ...
                //await sell.Products.map(async (product) => {
                for (const product of sell.Products) {
                    const createItensSellonDB = yield prismaTx.itensSell.create({
                        data: {
                            storeId: sell.UserId,
                    ...
                    if (sell.isDelivery) {
                        const deliveryAddress = yield prismaTx.address.create({
                            data: {
                                addressTypeId: 1,
                        ...
                        yield prismaTx.deliveries.create({
                            data: {
                ...
                for (const payment of sell.Payment) {
                    yield prismaTx.paymentSell.create({
                        data: {
                            storeId: sell.UserId,
                            sellId: createSellonDB.id,
