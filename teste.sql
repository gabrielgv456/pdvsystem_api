...
WHEN (CAST(COALESCE(:DIASPRI, 0) AS INTEGER) <> 0) AND (((CAST(COALESCE(:DIASPRI, 0) AS INTEGER)) > :CARENCIAAPRAZO) OR (((CAST(COALESCE(:DIASPRI, 0) AS INTEGER)) < :CARENCIAAPRAZO)) ) THEN
...
(CAST(
  (1+0)*(1/(
    (
      (
        POWER(1+(CAST(:I AS DOUBLE PRECISION)/100),
        CAST(:NUM_PERIODO AS DOUBLE PRECISION))-1
      )
      /
      (
        (CAST(:I AS DOUBLE PRECISION)/100)*
        (POWER((1+CAST(:I AS NUMERIC(15,4))/100),(CAST(:NUM_PERIODO AS DOUBLE PRECISION)+
        (CAST(:DIASPRI AS DOUBLE PRECISION)/30))))
      )
    )
    )) AS DOUBLE PRECISION
  )) 
  
  * CAST(:VAL_FINANCIADO AS DOUBLE PRECISION)
