/*
  ...
  - A unique constraint covering the columns `[storeId,id]` on the table `TaxGroup` will be added. If there are existing duplicate values, this will fail.
  - Added the required column `storeId` to the `TaxGroup` table without a default value. This is not possible if the table is not empty.
...
-- AlterTable
ALTER TABLE "TaxGroup" ADD COLUMN     "storeId" INTEGER NOT NULL;
...
-- CreateIndex
CREATE UNIQUE INDEX "TaxGroup_storeId_id_key" ON "TaxGroup"("storeId", "id");

-- AddForeignKey
ALTER TABLE "TaxGroup" ADD CONSTRAINT "TaxGroup_storeId_fkey" FOREIGN KEY ("storeId") REFERENCES "User"("id") ON DELETE RESTRICT ON UPDATE CASCADE;
